...
parser.add_argument("-d", "--dataset", type = str, default = "Musical_Instruments_5", help = "Dataset (Default: amazon_instant_video)")
...
parser.add_argument("-minReviews", "--minReviews", type = int, default = 5, help = "Minimum Reviews Per User/Item (Default: 1)")
parser.add_argument("-v", "--vocab", type = int, default = 40000, help = "Vocabulary Size (Default: 50000)")
...
parser.add_argument("-dev_test_in_train", dest = "dev_test_in_train", type = int, metavar = "<int>", default = 1,
...
user_count, item_count = count(interactions)  # 会去重
num_reviews = len(interactions)  # 返回列表中元素的总数
...
with codecs.open(REVIEW_JSON, 'r', encoding = 'utf-8', errors = 'ignore') as inFile:
	...
	for line in tqdm(lines, "Second pass of reviews for \"{}\"".format( CATEGORY )):
		...
		# if (hit( (user, item), users_dict, items_dict )): # 判断 (user, item) 对是否存在于 users_dict 和 items_dict 字典中
		#
		# 	if("Yelp" in CATEGORY):
		# 		rating = d['stars']
		# 		date = d['date']
		...
		if (hit((user, item), users_dict, items_dict)):
			try:
				...
				# 处理缺失的键的情况
				print("Missing key in the data dictionary.")
...
#########################################################
for user, uid in user_uid.items():
    print("User:", user, "UID:", uid)

for item, iid in item_iid.items():
    print("Item:", item, "IID:", iid)
##########################################################
